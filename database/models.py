...
class DbUser(Base):
    ...
    profile_image = relationship('DbProfileImage',uselist=False, cascade='all, delete', back_populates='user')
...
class DbProfileImage(Base):
    __tablename__ = 'profileimages'
    id = Column(Integer, primary_key=True)
    image_url = Column(String)
    timestamp = Column(DateTime)
    user_id = Column(Integer, ForeignKey('user.id'))
    user = relationship('DbUser', back_populates='profile_image')
