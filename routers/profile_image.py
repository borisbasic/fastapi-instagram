...
def post_profile_image(request: ProfileImage, db: Session = Depends(get_db), current_user: UserAuth = Depends(get_current_user)):
    return db_profile_image.post_profile_image(db, request)

@router.post('/image')
def upload_profile_image(image: UploadFile = File(...), current_user: UserAuth=Depends(get_current_user)):
